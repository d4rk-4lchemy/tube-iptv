...
## Weekly programmes and civil-time scheduling — 2026-09-24

- `uv run pytest -q`: **131 passed**. New coverage includes legacy SQLite migration and foreign keys, independent programme/source pools, weekly and overnight conflicts, programme-level XMLTV, source cancellation, removal/retention, clock continuity after idle/restart, duration learning with already buffered media, bounded overruns and live inputs.
- Timezone tests cover Warsaw's skipped spring starts, nonexistent ends, both autumn emissions, interruption at a repeated start even across a short gap, and Lord Howe's 30-minute change. EPG/calendar intervals remain contiguous and non-overlapping. These are controlled-clock tests, not a claim of observing a live seasonal clock change.
- `TUBE_URL=http://127.0.0.1:8003 uv run python scripts/programmes-check.py`: passed on a disposable instance with local generated media. Decoded output confirmed black RGB pixels and silent PCM, with H.264/AAC transport; newly available programme media replaced black, repeated clips kept monotonic non-overlapping media times and bounded RAM, source removal returned to black, and deleting the last programme restored rotation.
- `scripts/programme-boundaries-check.py`: passed on a separate empty test instance. Real FFmpeg output crossed general-source gap → one-minute programme → gap. The gap video was bounded by the programme start; the return respected the programme's overrun allowance. Natural transitions did not revoke the HLS stream, and accepted segment identities/timestamps remained consistent.
- `scripts/programmes-ui-check.mjs`: passed in Chromium. Verified form creation, dragging Wednesday out of an everyday rule, rejected overlaps, the gap preference, both 02:00 labels in a 25-hour calendar day, and desktop/mobile layout without JavaScript errors. Screenshots: local ignored `artifacts/programmes-desktop.png` and `artifacts/programmes-mobile.png`.
- Existing `scripts/ui-check.mjs` passed for channels, settings, EPG horizon/export/copy/error recovery, stable/nightly selectors and responsive layout. Existing `BROWSER_CHECK=1 scripts/smoke.py` passed on another disposable instance: real yt-dlp/FFmpeg, shared viewer position, H.264/AAC 1080p, multi-clip HLS continuity, Chromium playback and idle RAM release.
- Test integrations used isolated containers on ports 8003–8005. They did not change the user's local sources/settings. External YouTube availability, every supported yt-dlp extractor, hardware encoders and third-party IPTV devices are outside this verification.
- Local deployment completed with `sudo -n docker compose up -d --build tube`; image `b9455c5222bb`, container healthy. Read-only checks confirmed the new programme/calendar API, served frontend module and XMLTV (720 entries at verification), plus SQLite integrity and foreign keys. All existing channel/source/media identities were preserved: **1 channel, 3 sources, 264 media entries**. An existing viewer reconnected and the channel returned to `live`; normal extraction corrected one stored duration from 262 to 261 seconds.
- Before migration, SQLite's online backup API produced `/data/backups/before-programmes-20260924T103416Z.sqlite3`, also copied to ignored local `.data/backups/`. The backup passed its integrity check. Disposable test containers and their volumes were removed; the application data volume was retained.
